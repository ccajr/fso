...
import {
  ...
  Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions, Link, Box
} from '@mui/material'
  ...
  return (
    <Card sx={{ mt: 2, maxWidth: 600 }}>
      <CardContent>
        ...
        <Typography variant='subtitle1' sx={{ color: 'text.secondary' }}>by {blog.author}</Typography>
        <Link href={blog.url} target="_blank" rel="noopener" display="block" sx={{ mb: 1 }}>
          {blog.url}
        </Link>
        <Typography variant='body2' gutterBottom sx={{ color: 'text.secondary' }}>Added by {blog.user.name}</Typography>
      </CardContent>
      <CardActions sx={{ marginLeft: 8 + 'px', marginTop: -16 + 'px' }}>
        ...
        {isCreator && <Button variant='outlined' size='small' color='error' onClick={() => setConfirmOpen(true)}>remove</Button>}
      </CardActions>
